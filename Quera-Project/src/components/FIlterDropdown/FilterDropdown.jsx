...
import { ChooseFilter } from './ChooseFilter';
 ...
 export const FilterDropdown=({show,setShow})=>{
   ...
   function selectTag(id,value){
      setFilter(filter.map((item)=>{
         return  {id:item.id,tag:id==item.id ? value:item.tag,type:item.type,decide:item.decide}
      }))
   }
   function selectType(id,value){
      setFilter(filter.map((item)=>{
         return  {id:item.id,tag:item.tag,type:id==item.id ? value:item.type,decide:item.decide}
      }))
   }
   function DeleteFilter(id){
      setFilter(filter.filter((item)=>{return id!=item.id }))
   }
    return <section dir="rtl" id="Filter" style={{display: show ? "flex":"none"}}  className=" w-[718px] bg-white z-10  mt-6  shadow-lg rounded-lg font-dana   py-[15px] px-[21px] absolute flex-col ">
         ...
         {filter.map((item)=>{
            return <ChooseFilter DeleteFilter={DeleteFilter} selectTag={selectTag} selectType={selectType} selectDecide={selectDecide} id={item.id} key={item.id} tag={item.tag} type={item.type} decide={item.decide} />
         })}
