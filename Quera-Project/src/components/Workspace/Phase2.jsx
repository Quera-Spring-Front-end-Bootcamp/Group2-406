...
,"#F1A25C","#E28A60","#6897C2","#74AADD","#3C45E7","#6DAFCE","#6CB2F7","#9286EA","#C074D1","#486774","#5F6C7C","#46494D","#7FA1D1"]
    ...
    <div className="inline-flex justify-center  w-full flex-row ">
        <span className="  justify-self-center font-extrabold text-2xl tracking-tight font-dana  self-center">انتخاب رنگ ورک‌اسپیس</span></div><span onClick={()=>{setphase(1)}} className="float-left cursor-pointer text-2xl -translate-y-1 ">&larr;</span></div>
    <div className=" flex  flex-row box-border  mb-15 w-full"><div style={{backgroundColor: watch("color") ? watch("color"):"#7D828C",height:"70px",width:"70px" }} className=" ml-5 rounded-lg  justify-center items-center flex text-center font-dana text-2xl pt-auto   "><p className="text-center text-white align-middle font-bold">{watch("name").trim()[0]+" "+ (partedname[1]? partedname[1][0]:"")}</p></div><div className=" flex  flex-col gap-y-5   px-1 "  ><p className=" font-semibold font-dana text-sm">رنگ ورک‌اسپیس</p>
    <div  className="grid w-72 items-center gap-y-2.5 gap-x-2.5 grid-cols-12 grid-rows-2 "><input className="appearance-none hidden " {...register("color",)}  value={"#7D828C"} type="radio"  id="no1" /><label className=" h-4 w-4   cursor-pointer" htmlFor="no1">
